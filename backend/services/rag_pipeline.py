...
class RAGPipeline:
    ...
    async def retrieve(
        ...
        valid_vectors = [r for r in vector_results if r.get("score", 0) > THRESHOLD_MINIMUM]
        
        # 3. Strategy Decision
        strategy, sources = await self._decide_strategy(query, valid_vectors, max_results)
    ...
    async def _search_vectordb(self, query: str, user_id: str, top_k: int) -> List[Dict]:
        try:
            emb = await get_embedding(query)
            return await self.mm.search(user_id, emb, top_k)
        except: return []

    async def _decide_strategy(self, query: str, vector_results: List[Dict], max_results: int) -> Tuple[str, List[Dict]]:
